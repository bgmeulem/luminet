...
@pytest.mark.parametrize(
    "radius", [np.random.randint(6., 60.0) for _ in range(N_radii)]
)
@pytest.mark.parametrize(
    "order", [0.0, 1.0, 2.0]
...
class TestParametrized:
    ...
    def test_calc_periastron(self, mass, inclination, angle, radius, order):
        ...
        """
        p = bhmath.calc_periastron(
            radius=radius * mass, incl=inclination, alpha=angle, bh_mass=mass, order=order
        )
        return None

    def test_get_b_from_periastron(self, mass, inclination, angle, radius, order):
        b = bhmath.calc_impact_parameter(
            radius=radius*mass, incl=inclination, alpha=angle, order=order, bh_mass=mass
        )
